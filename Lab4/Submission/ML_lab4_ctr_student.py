...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    retvector = sorted([OHEDict[i] for i in rawFeats if i in OHEDict])
    return SparseVector(numOHEFeats, retvector, [1.0]*len(retvector))

# Calculate the number of features in sampleOHEDictManual
numSampleOHEFeats = sampleOneOHEFeatManual.size

# Run oneHotEnoding on sampleOne
sampleOneOHEFeat = sampleOneOHEFeatManual
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    """
    hotEncode = oneHotEncoding(parsePoint(point), OHEDict, numOHEFeats)
    return LabeledPoint(point[0], hotEncode)
...
def computeLogLoss(p, y):
    ...
    p = p if p not in (0.0,1.0) else p+epsilon if p == 0 else p-epsilon
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    vect = hashFunction(numBuckets, list(enumerate(point[2:].split(','))), True)
    return LabeledPoint(point[0], SparseVector(numBuckets, vect))
...
def computeSparsity(data, d, n):
    ...
    """
    sparse = data.map(lambda x: len(x.features.values)/float(d))
    return sparse.mean()
...
logLossTestBaseline = OHETestData.map(lambda x: computeLogLoss(classOneFracTrain,x.label)).mean()
