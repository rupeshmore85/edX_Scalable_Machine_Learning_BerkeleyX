...
def parsePoint(line):
    ...
    return LabeledPoint(ret[0], ret[1:])
...
averageTrainYear = (parsedTrainData
                    .map(lambda x: x.label).sum())/(parsedTrainData.map(lambda x: x.label).count())
...
def twoWayInteractions(lp):
    ...
    """
    feat = lp.features
    twowaylist = list(itertools.product(feat, repeat=2))
    twoway = np.hstack((feat, list((a*b for a,b in twowaylist))))
